...
<?php
    ...
    // قم بالحصول على رابط الصفحة التي تريد الانتقال إليها
    $destinationUrl = route('pdf.link', $orders->id); // حيث 'page' هو اسم الطريق
   
    ?>
<script>
  // فتح طلب الإشعارات
function askForNotificationPermission() {
  // تحقق مما إذا كان المتصفح يدعم إرسال الإشعارات
  if ('copy' in window) {
    Notification.requestPermission().then(function (permission) {
      // إذا تم قبول طلب الإشعارات
      if (permission === 'granted') {
        // عند النقر على الزر، أرسل الإشعار
        document.getElementById('copy').addEventListener('click', function () {
          new Notification(' تم نسخ الرابط يرجى الذهاب الى الواتس اب والقيام بعملية اللصق هناك ثم ارسال للعميل');
        });
      }
    });
  }
...
// استدعاء الدالة لطلب إذن الإشعار
askForNotificationPermission();
  ...
  function copyValue(event) {
      ...
      // الحصول على القيمة من العنصر الذي تم النقر عليه
      var valueToCopy = event.target.getAttribute('value');
      
      // إنشاء عنصر نصي غير مرئي لتحديد النص
      ...
      input.select();
      input.setSelectionRange(0, 99999); // للتأكد من تحديد جميع النصوص في حال كانت نافذة التحرير كبيرة
...
<body>
    ...
    <div class="card-body">
      ...
      <a value='{{ $destinationUrl }}' class="btn btn-danger btn-sm float-left export-btn" style="margin-left: 30px;color:white" data-toggle="tooltip" id="copy" data-placement="bottom" onclick="copyValue(event); askForNotificationPermission();"><i class="fas fa-export"></i> link</a>
